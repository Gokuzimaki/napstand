<?php
	...
	function getSingleContentEntry($id, $trucount="",$appuserid="",$usereditid=""){
		...
		/*go fetch the images for this content entry*/
			...
			if($mediacount>0){
				...
				while ($mediarow=mysql_fetch_assoc($mediarun)) {
					...
					$contentout[]= array(
										'largecover' => "$originalimage",
										'mediumcover' => "$medimage",
										'thumbcover' => "$thumbimage",
									);
					if($img_count==1){
						$firstpage= array(
										'largecover' => "$originalimage",
						...
						$firstoriginalimage=$originalimage;
						$firstmediumimage=$medimage;
						$firstthumbimage=$thumbimage;
					}else if ($img_count==$mediacount) {
						# code...
						$lastoriginalimage=$originalimage;
						$lastmediumimage=$medimage;
						$lastthumbimage=$thumbimage;
						$lastpage= array(
										'largecover' => "$originalimage",
										'mediumcover' => "$medimage",
										'thumbcover' => "$thumbimage",
		...
		';
		$row['selectionoutput']='<option value="'.$row['id'].'">'.$row['title'].'</option>';
		$row['adminoutput']='
			<tr data-id="'.$id.'">
